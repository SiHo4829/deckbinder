 ...
 *
 * P(프로모)만 사다리 밖의 45다. 봉입이 아니라 배포 경로가 다른 라벨이라
 * 사다리 칸 위에 올리면 그 자체가 서열 주장이 된다. 이 표에서 신뢰도가 가장
 * 낮은 값이고, 실데이터를 볼 때 첫 번째로 고칠 값이다 (§4.13 ⓓ-6의 6).
 *
 * 2026-09-03, 사용자가 60에서 45로 정했다 — 「프로모는 봉입 상위 라벨보다는
 * 구하기 쉽다」는 취지다. 45는 사다리 칸 위가 아니라 칸 사이(R 35 · SR 55
 * 사이)라는 성질을 유지한다. 이 값도 실측이 아니라 판단이므로 위 문장(신뢰도가
 * 가장 낮고 첫 번째로 고칠 값)은 그대로 유효하다.
 *
