...
| 단계 | 에이전트 | 하는 일 | 고치는 것 |
|---|---|---|---|
| 설계 · **지휘** | **architect** | 요구 분석 · 태스크 분해 · 모듈/스키마/API 계약 확정 · 다음 단계 지시 · **완료 축 아카이브** | `.claude/plan.md` · `.claude/plan-archive.md` |
...
- **완료된 축은 architect가 *닫으면서* `plan-archive.md`로 옮긴다.** 태스크를 닫는 절차의 일부이지 별도 청소가 아니다. **묵혀 두면 `plan.md`가 커지고, 커지면 에이전트가 필요한 절을 못 찾아 넓게 훑는다** — 그것이 실제 토큰 비용이다.
  - **옮기는 것:** 태스크 본문(판정 · 🔒 고정 문안 · 완료 기준 · 멈춰야 하는 신호 · reviewer 표). 🚨 **원문 그대로 옮긴다 — 요약하지 않는다.** 아카이브는 「안 읽는 곳」이지 「없어진 곳」이 아니다.
  - **`plan.md`에 남기는 것:** 체크박스 줄(상태) · 포인터 한 줄 · 🚨 **그 축이 낳은 *살아 있는 규칙*.**
  - 🚨🚨 **가르는 기준은 하나다: 「이 문장이 다음에 누군가가 같은 실수를 하는 것을 막는가?」 그렇다면 `plan.md`에 남긴다.** ⚠️ **예: T5.x는 닫혔지만 「비영리를 정당화 근거로 쓰지 않는다」·「폐기된 분당 3회를 이미지 프록시에 섞지 않는다」는 지금도 걸린다** — 함께 옮겼으면 안 읽히게 되고, 그것이 `§5.6`이 막으려는 실패다.
  - 🚨 **체크박스로 자동화하지 않는다.** **구현이 끝나도 체크박스는 꺼져 있을 수 있다**(커밋·닫는 절차가 남아서) — T5.x·T6.x가 그 상태였다. **판정은 architect가 한다.**
  - **인덱스를 함께 갱신한다** — `plan.md` 최상단 「이 파일을 읽는 법」의 표. 🚨 **행 번호를 적지 않는다**(편집 한 번에 어긋난다). grep 앵커로만 적고, 앵커가 유일하게 매치하는지 확인한다.
