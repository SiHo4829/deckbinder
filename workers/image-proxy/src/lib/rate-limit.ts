 ...
 *
 * 🚨 **§5.4의 「분당 3회」를 여기 적용하지 않는다.** 그것은 **되팔이 방지
 * 쿼터**이고 이것은 **남용 방지 리밋**이다 — `CLAUDE.md`가 갈라 둔 두 규칙을
 * 섞지 않는다. 그 둘을 섞으면 세트 화면 한 장(60장)이 즉시 막힌다.
 *
...
/**
 * IP를 **솔트 해시로만** 다룬다 (§5.4의 규칙 그대로).
 *
 * 🚨 시크릿 이름은 `IMAGE_PROXY_IP_SALT`이고 **T2.9의 `IP_HASH_SALT`와 같은
 * 값을 쓰지 않는다** — §9.2 ⓐ의 "값을 나누는 비용은 0이다".
 *
