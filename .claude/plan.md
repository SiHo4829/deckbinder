...
문의처는 `card@xosoft.kr`(§9.3에서 확인한 `onepiece-cardgame.kr`의 유일한 공개 창구)이다. **회신이 오면 그 내용을 `docs/crawler-compliance.md` §3에 원문 그대로 인용해 남긴다** — 그 시점에 이 결정의 근거가 "근거 없음"에서 실제 근거로 바뀐다.
...
- **ⓒ 권리자 문의 메일 발송** (§4.4.1 결정 6 — `card@xosoft.kr`). **T1.14를 막지 않는다. 병행이다.** 배포 전에 보내는 것이 요점이고(협상 위치), 회신은 §9.1 ⑤가 받는다. 회신이 오면 `docs/crawler-compliance.md` §3에 원문으로 남긴다
...
- **Phase 2 착수** — "카드가 없으니 코드나 짜자"는 유혹이 가장 센 자리지만 **Phase 2가 데이터에 더 굶주려 있다.** T2.4·T2.5는 카드가 있어야 화면이 성립하고, T2.7은 **§9.3 ⓒ(메르카리·라쿠마·야후옥션 약관 — 여전히 미검토)**가 선행이며 검색 키인 `name_ja`를 가진 카드가 0장이면 조회 대상 자체가 없다. **예외는 T2.1 · T2.2** — 순수 함수라 DB도 데이터도 필요 없다. 손입력이 견디기 어려울 때의 **피신처**로만 남겨 둔다
