...
| 손입력 참조 원천은 **`onepiece-cardgame.kr` 하나**. 일본 2곳은 배제. **근거는 "허용 확인"이 아니라 "금지 근거를 찾지 못함"이다** | §4.4.1 |
...
| 4 | **이미지는 핫링크 유지** (§9.4) |
| 5 | **배포는 광고 없이 먼저 한다. 애드센스는 허락을 받은 뒤에 붙인다** (§9.1) |
| 6 | **권리자 문의 메일은 배포를 기다리지 않고 보낸다** |
...
> 포켓몬코리아에는 남는 조건이 하나 더 있다. §9.3 판정이 "중간"이었던 이유 — **재사용 금지 조항의 주어가 "회원"이라 비회원에게 그대로 걸리는지가 문언상 불명확하다.** 포켓몬을 넣기로 할 때 이 점과 §9.1 애드센스 순서를 함께 다시 본다.

**문의 메일은 배포를 기다리지 않는다 (결정 6).** 근거는 둘이고, 뒤가 더 중요하다.
...
**초안 4통은 `docs/permission-inquiry-drafts.md`에 있다.** 당장의 원천인 `onepiece-cardgame.kr`의 공개 창구는 `card@xosoft.kr`(§9.3에서 확인한 유일한 연락처)이고, 나머지 3곳은 지금 원천이 아니어도 **회신이 오면 결정 2·3을 다시 열 수 있으므로** 함께 보낸다. **회신이 오면 그 내용을 `docs/crawler-compliance.md` §3에 원문 그대로 인용해 남긴다** — 그 시점에 이 결정의 근거가 "근거 없음"에서 실제 근거로 바뀐다. **회신 유형별로 어디로 가는지는 §9.11에 있다.**

**이 결정을 되돌려야 하는 조건**

1. `onepiece-cardgame.kr`에 이용약관·저작권 안내가 새로 게시된다 (§9.3 ⓓ 재확인 절차에서 잡는다)
2. 문의에 **거절** 회신이 온다
3. 권리자로부터 중지 요청을 받는다
4. **광고를 붙이려는 시점** — 결정 5의 순서를 건너뛰면 이 결정의 전제가 깨진다
...
- [ ] **T1.14** 실제 카드 손입력 1배치 — 원피스 ST-01 · ST-02 (34종) · M
  ...
  - **이미지는 핫링크로 넣는다 (§9.4 확정).** 자체 저장소로 복제하지 않는다. URL도 `onepiece-cardgame.kr`에서 얻는다
  ...
  - **필드별 입력 출처** — `/admin/sets`에서 세트 2개, `/admin/keywords`에서 opcg 키워드를 **먼저** 만든 뒤 카드를 넣는다
    ...
    | `image_url` | 선택 | **`onepiece-cardgame.kr`의 이미지 URL을 핫링크** (§9.4 확정 · §4.4.1 결정 4) | `z.url()`만 통과하면 저장된다. `CardImage`가 `<img>`라 `remotePatterns` 설정은 불필요. **파일을 내려받아 우리 저장소에 두지 않는다** — 그것이 4곳 약관이 첫 번째로 지목한 「복제」다(§9.4). **핫링크라 원본이 사라지면 이미지도 사라진다** — 이번 배치는 그 상태를 그대로 겪어 보는 것이 목적이다 |
...
**다음은 T1.14(손입력)이고 실행은 사용자가 직접 한다.** 선행 태스크는 전부 닫혔고 `main`은 `origin`까지 push됐다. **입력 원천도 §4.4.1로 정해졌다 — `onepiece-cardgame.kr` 하나다.** 착수 전 남은 것은 셋이다.

- **ⓐ `ADMIN_TOKEN`을 43자 난수로 교체** (§9.2 ⓐ의 명령 그대로). **자격증명이라 사용자가 직접 넣는다.** **데이터가 들어가기 전에** 끝내야 하는 항목이고, 지금이 마지막 기회다 — 내일부터는 잃을 것이 생긴다
- **ⓑ 입력 직전 `npm run db:dump` 1회.** 0행 상태의 기준점을 남긴다
- **ⓒ 권리자 문의 메일 발송** — 초안 4통은 `docs/permission-inquiry-drafts.md`에 준비됐다(§4.4.1 결정 6). **T1.14를 막지 않는다. 병행이다.** 배포 전에 보내는 것이 요점이고(협상 위치), 회신은 §9.1 ⑤가 받는다. **보내면서 두 가지를 함께 정한다 — 발송일 기록과 무응답 기한(§9.11 ⓐ).** 기한을 안 정하면 대기가 무기한이 된다

**T1.14의 위험은 이제 두 가지로 줄었다.** 오타는 T1.15가, 소실은 T1.13이 받는다. 남은 것은 **입력 확인을 `/cards`가 아니라 `/admin/cards`에서 한다**는 것(TanStack Query `staleTime` 5분 — T1.14 블록의 주의)과, **측정 기록을 실제로 채우는 것**이다.
...
- **Phase 2 착수** — "카드가 없으니 코드나 짜자"는 유혹이 가장 센 자리지만 **Phase 2가 데이터에 더 굶주려 있다.** T2.4·T2.5는 카드가 있어야 화면이 성립하고, T2.7은 **§9.3 ⓒ(메르카리·라쿠마·야후옥션 약관 — 여전히 미검토)**가 선행이며 검색 키인 `name_ja`를 가진 카드가 0장이면 조회 대상 자체가 없다.
  - **예외는 T2.1 · T2.2이고, 2026-08-25에 성격이 올라갔다.** 순수 함수라 DB도 데이터도 필요 없다는 것은 그대로인데, **§9.11 ⓔ가 근거를 하나 더 줬다 — 문의 회신이 어느 쪽으로 오든 버려지지 않는 유일한 작업이다.** 카드 DB를 호스팅하지 않는 갈래(§9.11 ⓓ-2)에서도 그대로 쓰인다. 따라서 **"손입력이 견디기 어려울 때의 피신처"가 아니라 "회신을 기다리는 동안 하는 작업"이다.** T1.14와 경쟁하지 않는다 — T1.14는 사용자가 손으로 하고, 이쪽은 그 대기 시간에 들어간다
- **§9.1 애드센스** — 기사는 콘텐츠 작업이지만 **도메인 교체(§9.1 ②)와 `ads.txt`(③)가 정해지기 전에는 써 두어도 심사에 넣을 수 없다.** 도메인 결정이 먼저이고 30분짜리 판단이다. 기사 소재도 카드 데이터가 있어야 나오므로 순서상 T1.14 뒤다. **2026-08-25로 막는 것이 하나 더 붙었다 — §9.1 ⑤(권리자 회신).** 준비물 ①~④는 미리 채워도 되지만 **제출은 회신 뒤다.** 즉 애드센스는 이제 "언제 할지"가 우리 일정만으로 정해지지 않는다
...
1. **애드센스 심사 제출 전 준비물** — ①실제 기사 5~10편 발행(코드가 아니라 콘텐츠 문제) ②`NEXT_PUBLIC_SITE_URL`을 실제 도메인으로 교체 ③`ads.txt` 배치와 퍼블리셔 ID 입력 ④EEA 트래픽이 있으면 인증 CMP 도입. ~~플레이스홀더 페이지가 "제작 중"으로 보이는 것~~ → T1.10에서 `ComingSoon`으로 해소. **§2.7의 데이터 캐시 문제는 여기에 실질 위험이 아니다.** T1.12-7 뒤 색인 대상인 `/news/[slug]`와 `/cards/[cardId]`는 **동적이라 항상 최신**이다. 지연이 남는 곳은 **`sitemap.xml`(3600) 하나이고 그건 알고 남긴 것**이다 — 크롤 주기가 시간 단위 이상이라 1시간 늦게 실린 URL이 색인 결과를 바꾸지 않고, 내려간 URL이 1시간 더 실려 있어도 크롤러가 404를 받아 스스로 뺀다. **조치 없음.** 재평가는 실제 색인 제출 후 Search Console에서 지연이 관측되면 한다.
   ...
   **⑤ 권리자 회신 — 2026-08-25 §9.3 검토로 추가되고, 같은 날 §4.4.1로 순서가 확정된 항목.** 위 ①~④는 전부 애드센스 쪽 요건인데, 이건 **반대 방향의 요건**이다. 문제는 애드센스가 아니라 **"약관이 재사용을 허락하지 않은 데이터"와 "광고"가 한 페이지에 같이 있는 것**이므로 **심사에 통과해도 해소되지 않는다.**

   **순서가 확정됐다 (§4.4.1 결정 5·6). 이 순서를 건너뛰면 §4.4.1의 전제가 깨진다.**

   ```
   T1.14 손입력 → 배포(광고 없음) → 문의 메일 → 회신 → 애드센스
                        └──────────┘
                   문의는 배포를 기다리지 않고 병행한다 (§4.4.1 ⓐⓑ)
   ```

   - **배포를 광고 없이 먼저 하는 이유** — 현재 원천(`onepiece-cardgame.kr`)의 4축 판정이 전부 "근거 없음"이라 영리성 여부를 판단할 재료가 없다. **광고를 빼면 최소한 ④축을 논쟁에서 제외한 채 시작할 수 있다**
   - **회신을 기다리는 이유** — ⑤가 닫히기 전에는 ①~④를 다 채워도 붙이지 않는다. 순서상 ①(기사)·②(도메인)·③(`ads.txt`)를 준비하는 것은 무방하나 **제출은 회신 뒤다**
   - **포켓몬을 넣게 되면 이 항목을 다시 본다** — 포켓몬코리아는 조건이 「영리목적」이라 **광고를 붙이는 순간 정면으로 걸린다**(§4.4.1의 표). 즉 포켓몬 카탈로그와 애드센스는 서로를 막는 관계다
...
3. **외부 사이트 약관 — 카드 정보 4곳 검토 완료 (2026-08-25) · 중고 매물 3곳 미검토.**
   ...
   **ⓒ 미검토 — 메르카리 · 라쿠마 · 야후옥션 (T2.7 선행).** 이번 범위가 아니며 **여전히 미검토로 남는다.** 성격이 다르다 — 카탈로그가 아니라 매물 시세를 읽고(§5.3), 세 곳 모두 로그인·약관 동의를 전제한 거래 플랫폼이라 자동화 금지 조항이 명시돼 있을 가능성이 카드 정보 4곳보다 훨씬 높다. **T2.6 착수 전에 같은 4축 형식으로 기록한다.** 차단 시 대체 전략(공식 API · 제휴)이 필요하다.

   **ⓓ 재확인 의무.** 약관은 바뀐다. `docs/crawler-compliance.md` §9에 확인 URL과 날짜 표를 뒀다. **새 입력 배치를 시작하기 전에 그 표의 URL을 다시 열어 인용문과 대조한다.**
4. **카드 이미지 저장 방식 — 결정 완료 (2026-08-25): 핫링크 유지.** 관리자가 외부 URL을 직접 입력하는 현재 방식을 그대로 둔다(§4.4.1 결정 4). 원래 질문은 "핫링크냐 자체 호스팅(Supabase Storage / R2)이냐"였고 판단 재료를 T1.14가 만들기로 했는데, **그 재료는 운영 측면(URL을 안정적으로 구할 수 있는가)이었다.** §9.3 약관 검토가 다른 축의 재료를 줬고 결론이 뒤집혔다.
   ...
   **자체 호스팅은 운영 편의를 올리는 대신 위험을 명확히 키우는 방향이므로, 허락을 받은 뒤에만 검토할 선택지로 내린다**(§4.4.1 결정 6의 회신이 그 허락이다). 그리고 **원래 §9.4에 없던 세 번째 선택지를 남겨 둔다: "이미지를 싣지 않는다".** ③이 4축 중 위험이 가장 집중된 곳인데 **덱 구성 · 기준가 · 키워드 검색은 이미지 없이도 성립한다.** 최소한 **`image_url`이 선택적이고 없어도 서비스가 동작하는 상태**는 유지한다 — 이것이 이 결정의 되돌릴 수단이다.

   > **미결로 남는 것은 "핫링크냐 자체 호스팅이냐"가 아니라 "핫링크냐 아예 빼느냐"다.** 앞의 갈래는 닫혔고, 뒤의 갈래를 T1.14의 실측(원본 URL의 안정성 · 장당 확보 시간)이 정한다.

   **`CardImage`가 `next/image`가 아니라 `<img>`인 것은 이 결론과 맞으므로 그대로 둔다** — `next/image`는 원본을 가져와 최적화본을 만들어 캐시하므로 **동작 자체가 복제에 가깝다.** `images.remotePatterns`가 필요 없다는 것은 부수 효과일 뿐이다. T1.14가 만들 운영 수치(URL 확보가 장당 입력 시간의 몇 할인지)는 **"이미지를 싣지 않는다"와 핫링크 사이를 가르는 데** 쓴다.
...
11. **문의 회신별 대응 — 시나리오만 (2026-08-25).** §4.4.1 결정 6의 문의(초안 4통은 `docs/permission-inquiry-drafts.md`)에 회신이 왔을 때 **그 자리에서 판단하지 않기 위한** 갈래다. **태스크로 만들지 않는다** — 아직 일어나지 않았고 갈래마다 할 일이 완전히 다르다. 갈래를 지금 설계하지도 않는다. **어떤 회신이 오면 어디로 가는지만** 적는다.

    | 회신 | 어디로 |
    |------|--------|
    | 허락 | §4.4.1 원천 표에서 해당 사이트를 ✅로. 그 사이트 범위에서 §9.4(자체 호스팅)와 §9.1 ⑤(애드센스)도 함께 풀린다 |
    | 조건부 허락 (출처 표기 · 비영리 한정 등) | 조건을 §4.4.1에 **원문 그대로** 적고 표를 갱신. 조건이 비영리면 §9.1 ⑤는 닫힌 채 남는다 |
    | 거절 | 해당 사이트를 ❌로 닫는다. 4곳이 전부 닫히면 → **ⓓ** |
    | **무응답** | **가장 흔한 결과다** → **ⓐ** |
    | 중지 요청 | 해당 원천의 데이터를 즉시 내린다. 되돌릴 수단은 T1.13 덤프 |

    **ⓐ 무응답 기한을 정해야 한다.** 대기업 법무·홍보는 이런 문의에 대체로 답하지 않는다. **기한 없이 두면 "아직 기다리는 중"이 무기한이 되고, 결정을 미루는 것이 결정처럼 보인다.** 기한(예: 4주)과 만료 시 어느 갈래로 갈지는 **사용자가 정할 몫이고, 여기서는 정해야 한다는 사실만 남긴다.** 기산점을 잃지 않도록 **발송일을 `docs/permission-inquiry-drafts.md`에 적는다.**

    **ⓑ 묻는 데에는 비용이 있고, 알고 선택했다.** 거절 회신을 받고도 진행하면 **애초에 묻지 않았을 때보다 위치가 나빠진다.** 그럼에도 물은 이유는 둘이다 — 묻지 않는다고 해서 합법이 되는 것이 아니고, §4.4.1의 근거가 "금지 근거를 찾지 못함"이라 **그 공백을 메울 방법이 문의뿐**이다. 나중에 "왜 굳이 물어봤나"가 나올 질문이라 여기 남긴다.

    **ⓒ 부분 허락이 정상이다.** 4곳이 한꺼번에 답하지 않는다. 원피스 한국만 허락, 일본만 거절 같은 조합이 실제로 나온다. **§4.4.1의 원천 표를 사이트별로 갱신하는 것으로 족하다** — 결정이 애초에 사이트 단위로 쓰여 있다. **방침 자체가 바뀌는 조합은 하나뿐이다: 4곳이 전부 닫히는 경우(→ ⓓ).**

    **ⓓ 전부 막혔을 때 — 세 갈래. 지금 고르지 않는다.**
    ...
    **ⓔ 모든 갈래에서 살아남는 것 — T2.1(시뮬레이터) · T2.2(덱 검증).** 순수 함수라 카드 DB가 있든 없든, 데이터를 우리가 갖든 사용자가 입력하든 똑같이 필요하다. **ⓓ-2에서도 그대로 쓰인다.** 즉 **어떤 회신이 와도 버려지지 않는 유일한 작업이고, 그래서 회신을 기다리는 동안 할 작업이다.**
