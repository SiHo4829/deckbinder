...
> ⚠️ **2026-08-25 — 손입력이라고 해서 다 같지 않다.** §9.3의 약관 검토 결과, **재사용 금지 조항은 수집 방법을 조건으로 달지 않는다.** 즉 자동 수집을 그만둔 것으로 벗어난 것은 **접근 규율 하나뿐**이고, 데이터 재사용·이미지·상업적 이용은 그대로 남는다. 조항들은 「このサイトに掲載されている」·「본 사이트의 콘텐츠」처럼 **사이트 게재물**을 대상으로 쓰여 있으므로 **원천이 갈린다.**
>
> | 원천 | 4곳의 약관에 걸리는가 |
> |------|----------------------|
> | 카드 실물을 보고 입력 | **범위 밖** (실물에 대한 별도 권리 문제는 남는다) |
> | 공식 사이트 화면을 보고 입력 | **걸린다** |
> | 자동 수집 | 걸린다 + 접근 규율까지 |
>
> **T1.14가 어느 원천에서 입력할지는 정해져 있지 않다 — 착수 전에 사용자가 결정해야 한다.** 근거와 인용문은 `docs/crawler-compliance.md` §6.
...
**다음은 T1.14(손입력)이고 실행은 사용자가 직접 한다.** 선행 태스크는 전부 닫혔고 `main`은 `origin`까지 push됐다. **착수 전 두 가지만 남았다.**
...
1. **애드센스 심사 제출 전 준비물** — ①실제 기사 5~10편 발행(코드가 아니라 콘텐츠 문제) ②`NEXT_PUBLIC_SITE_URL`을 실제 도메인으로 교체 ③`ads.txt` 배치와 퍼블리셔 ID 입력 ④EEA 트래픽이 있으면 인증 CMP 도입. ~~플레이스홀더 페이지가 "제작 중"으로 보이는 것~~ → T1.10에서 `ComingSoon`으로 해소. **§2.7의 데이터 캐시 문제는 여기에 실질 위험이 아니다.** T1.12-7 뒤 색인 대상인 `/news/[slug]`와 `/cards/[cardId]`는 **동적이라 항상 최신**이다. 지연이 남는 곳은 **`sitemap.xml`(3600) 하나이고 그건 알고 남긴 것**이다 — 크롤 주기가 시간 단위 이상이라 1시간 늦게 실린 URL이 색인 결과를 바꾸지 않고, 내려간 URL이 1시간 더 실려 있어도 크롤러가 404를 받아 스스로 뺀다. **조치 없음.** 재평가는 실제 색인 제출 후 Search Console에서 지연이 관측되면 한다.
   ...
   **⑤ 카탈로그 데이터의 원천 정리 — 2026-08-25 §9.3 검토로 추가된 항목.** 위 ①~④는 전부 애드센스 쪽 요건인데, 이건 **반대 방향의 요건**이다. 카드 정보 4곳 중 3곳이 허용 범위를 「個人的に楽しむ場合」(포켓몬 일본) · 「私的使用…を超えて」(반다이) · 「영리목적」「상업적으로 이용하는 행위」(포켓몬코리아)로 그었다 — **광고를 붙이는 순간 이 선을 넘는 쪽으로 이동한다.** 문제는 애드센스가 아니라 **"약관이 영리 이용을 금지한 데이터"와 "광고"가 한 페이지에 같이 있는 것**이므로, **심사에 통과해도 해소되지 않는다.** 지금 당장 조치는 불필요하다(§9.8 실측: 전 테이블 0행). 다만 **결정 시점은 애드센스보다 앞인 T1.14다** — 손입력이 어느 원천에서 시작되는지가 곧 이 항목의 답이 된다(§9.3 ⓑ).
...
3. **외부 사이트 약관 — 카드 정보 4곳 검토 완료 (2026-08-25) · 중고 매물 3곳 미검토.**
   ...
   **ⓑ 가장 중요한 결과 — 손입력이 피해 주는 축은 ① 하나뿐이다.** §4.4가 "사람이 보고 옮겨 적는 것"을 허용한 것은 **접근 규율을 벗어난 것이지 ②를 푼 것이 아니다.** 인용한 재사용 금지 조항 어디에도 수집 방법이 조건으로 들어 있지 않다 — 「他のインターネットなどの公衆ネットワーク上で利用することはできません」(포켓몬 일본) · 「無断転用、転載」(원피스 일본) · 「복제, 송신, 출판, 배포, 방송 **기타 방법에 의하여**」(포켓몬코리아 제14조③). **즉 pokemon-card.com은 손입력이든 자동화든 문언상 동일하게 막힌다.**
   ...
   그래서 손입력에서도 **원천을 구분해야 한다.** 인용 조항은 모두 「このサイトに掲載されている」·「본 사이트의 콘텐츠」처럼 **사이트 게재물**을 대상으로 쓰여 있다 — **카드 실물을 보고 입력하는 것은 이 4곳의 약관 범위 밖이다**(실물에 대한 별도 권리 문제는 남지만 사이트 약관은 아니다). **T1.14의 원천을 어디로 둘지가 지금 결정할 사항이다.**
...
4. **카드 이미지 저장 방식 — 기본값을 "핫링크 유지"로 둔다 (2026-08-25 §9.3 검토로 뒤집힘).** 현재는 관리자가 외부 URL을 직접 입력한다. 원래 질문은 "핫링크냐 자체 호스팅(Supabase Storage / R2)이냐"였고 판단 재료를 T1.14가 만들기로 했는데, **그 재료는 운영 측면(URL을 안정적으로 구할 수 있는가)이었다.** 약관 검토가 다른 축의 재료를 줬고 결론이 바뀐다.
   ...
   **자체 호스팅은 운영 편의를 올리는 대신 위험을 명확히 키우는 방향이다 — 허락을 받은 뒤에만 검토할 선택지로 내린다.** 그리고 **§9.4에 없던 세 번째 선택지를 추가한다: "이미지를 싣지 않는다".** ③이 4축 중 위험이 가장 집중된 곳인데 **덱 구성 · 기준가 · 키워드 검색은 이미지 없이도 성립한다.** 최소한 **`image_url`이 선택적이고 없어도 서비스가 동작하는 상태**는 유지한다.
