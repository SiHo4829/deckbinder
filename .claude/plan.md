...
`으로 정규화하는 것은 `effect_text`에 무해하다(오히려 표준화된다). **「대가를 알고 받아들인 것」과 「어쩔 수 없는 것」을 섞지 않는다 — 전자는 되돌릴 수 있고 후자는 아니다.** `parse.test.ts` #12를 "원문 `\r\n` 보존"에서 "개행이 `\n`으로 유실 없이 남는다"로 조정했다. `src/lib/validation/catalog.ts`의 `\r\n`→`\n` 방어적 변환과 그 단위 테스트는 원안대로 유지했다(중간 파일 JSONL 자체에는 이제 `\r\n`이 없겠지만, 변환 로직은 idempotent하고 무해하다)
  - ⚠️ **실제 네트워크 수집은 실행하지 않았다** — T1.19 범위. `scripts/collect-catalog.ts`는 typecheck·lint만 통과했고(vitest의 `include`가 `src/**`라 `scripts/`는 단위 테스트 대상이 아니다 — ⓒ가 이미 이렇게 설계했다) 실행 자체는 검증하지 않았다
...
- [ ] **T1.19** 첫 실적재 1세트 + 측정 — **S (단 사람 손이 필요하다)** · 선행 T1.18 + 사용자 일감(범위 승인 · `db:dump` · §9.3 ⓓ 재확인)
  ...
  - **완료 기준**
    ...
    - ⓓ ★ **§9.4 ⓔ 측정 2·3을 잰다** — 폴백 발생률 · `no-referrer` **적용/미적용** 이미지 로드 실패율. **T1.14가 지고 있던 자리를 여기서 받는다**
    - ⓔ 백로그 **B-6의 미실측 항목**(진짜 죽은 URL에 대한 브라우저 동작 · SSR `<img>`의 하이드레이션 전 실패)을 **프로덕션 빌드로** 확인한다
...
| **오늘 미루는 것** | **T2.6 · T2.7** · **B-4 · B-5** · **C-1 · C-2** — 아래 「미루는 것」. ⚠️ **A-5는 이 줄에서 빠졌다** — 2026-08-28에 설계가 확정돼 **T1.16~T1.19로 착수 가능한 상태**가 됐다 |
