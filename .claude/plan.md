...
**착수 순서.** ~~ⓐ(`CLAUDE.md` 개정)~~ ✅ **닫혔다** → ⓒ(부하 규율 확정) → ⓑ-1(수집기 + 중간 파일) → **중간 파일을 눈으로 확인** → 스키마 공백 4건 판단 → 마이그레이션 → ⓑ-2(임포터, 드라이런 먼저) → 본 적재.
...
- [ ] **A-5** ~~CSV 일괄 등록~~ → **카탈로그 자동 수집 + 임포트 · L~XL** — ★ **2026-08-28에 성격이 바뀌었고 최우선이 됐다 (§0.1 ⓓ).** T1.14가 폐기되면서 **이것이 카탈로그를 채우는 유일한 경로**다
  - **2단으로 나눈다: 수집(→ 중간 파일) / 임포트(→ DB).** 한 번에 하면 네트워크 오류가 DB를 반쯤 채운 상태로 끝나고, 중간 파일이 있으면 **눈으로 보고 나서** 넣을 수 있다. 설계는 §0.1 ⓓ ⓑ
  - **선행이 바뀌었다.** ~~T1.14(입력 표본)~~은 없어졌고, 대신 **`CLAUDE.md` 크롤러 조항 개정(§0.1 ⓓ ⓐ — 사용자 승인)**과 **부하 규율 확정(ⓒ)**이 앞에 붙는다. **T1.13(되돌릴 수단)은 그대로 선행이고 비중이 올랐다** — 한 번에 들어오는 양이 34장이 아니다
  - **기존 미결은 하나도 해소되지 않았다** — 중복 처리 · 부분 실패 · **드라이런** · 게임/세트 매핑. **잘못 만들면 카탈로그를 덮어쓴다**(§4.4가 시드 스크립트를 삭제한 것과 같은 위험)
  - 🚨 **§9.2 ⓒ 전제 3(관리자 API의 파괴 표면 동결)을 착수 전에 재검토한다.** 자동 등록은 반복 실행이 전제라 **upsert를 거의 확실히 부른다**
  - **새 미결 둘**: ①**수집 코드가 어디서 도는가** — §1 P2가 「외부 접근은 Workers에서만」이라고 정했는데, 일회성 적재 스크립트도 그 대상인지가 쓰여 있지 않다 ②**원천 HTML 구조 의존** — 사이트 개편 시 조용히 깨지거나 잘못된 값을 넣는다. 임포트 단의 형식 검증에 `src/lib/validation/admin.ts`의 zod 스키마를 재사용할 수 있는지가 첫 확인 대상
...
| 4 | ~~T1.14 손입력~~ · ~~`CLAUDE.md` 개정 승인~~ → **둘 다 닫혔다 (2026-08-28).** 남은 사용자 몫은 **A-5 수집 범위 승인**(어느 세트부터, 몇 장까지) | 개정은 반영됐다(`CLAUDE.md` §2 (B)). **수집을 실제로 돌리기 전에 범위를 사람이 정해야 한다** — 부하 규율(§0.1 ⓓ ⓒ)의 「전체 요청 수 상한」이 그 값에서 나온다 |
