...
| 🚨 **착수 전 `CLAUDE.md`의 크롤러 조항 범위를 갈라 적어야 한다 — 매물 크롤러(1회 1장 유지)와 카탈로그 수집기는 다른 규율이다.** 사용자 승인 필요 | §0.1 ⓓ ⓐ ★ |
...
**ⓐ `CLAUDE.md`의 크롤러 조항 범위를 명시한다 — 이것이 blocker다.**
...
> **→ 그래서 이것은 "규칙 위반"이 아니라 "규칙의 범위가 쓰여 있지 않다"이다.** 그대로 두면 **다음 세션이나 새 에이전트는 문언대로 읽고 이 작업을 거부하거나, 반대로 매물 크롤러에까지 배치 수집을 허용해 §5.4를 무너뜨린다.** 둘 다 나쁘다. **`CLAUDE.md`를 고쳐 두 크롤러를 갈라 적는다** — ①**매물 검색 크롤러**: 1회 1장 · 배치 금지 **유지**(되팔이 방지라 데이터 경로와 무관하다) ②**카탈로그 수집기**: 별도 규율(아래 ⓒ). ⚠️ **`CLAUDE.md` 개정은 사용자 승인이 필요하다** — 이 문서가 고칠 수 있는 파일이 아니다.
...
**착수 순서 — 위 셋이 닫힌 뒤.** ⓐ(`CLAUDE.md` 개정, 사용자) → ⓒ(부하 규율 확정) → ⓑ-1(수집기 + 중간 파일) → **중간 파일을 눈으로 확인** → 스키마 공백 4건 판단 → 마이그레이션 → ⓑ-2(임포터, 드라이런 먼저) → 본 적재.
...
| 4 | ~~T1.14 손입력~~ → **폐기 (2026-08-28 · §0.1 ⓓ).** 대신 **`CLAUDE.md` 크롤러 조항 개정 승인** | **자동 수집으로 방침이 바뀌었다.** 사용자가 할 일은 옮겨 적는 것이 아니라 **`CLAUDE.md`를 고칠지 결정하는 것**이다 — 문언만 보면 「Do NOT implement batch scraping」이 이번 결정을 금지하는데, §5.4를 보면 그 조항의 실체는 **매물 검색 되팔이 방지**다. **범위를 갈라 적지 않으면 다음 세션이 문언대로 읽고 막힌다**(§0.1 ⓓ ⓐ) |
