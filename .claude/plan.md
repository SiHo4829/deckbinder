...
> **실행 순서는 번호순이 아니다 — `T1.13 → T1.15 → T1.14`.** T1.14(손입력)가 먼저 설계돼 번호를 가져갔고, 그 뒤 T1.15(세트·키워드 수정)를 앞에 놓기로 했다. 이유는 각 블록에 있다.
> - **T1.13이 없으면** T1.14의 결과물이 클릭 한 번에 사라진다 (§9.2 ⓑ)
> - **T1.15가 없으면** 손입력 도중 세트명 오타를 발견해도 고칠 수 없다 — 그때는 카드가 이미 걸려 있어 **세트 삭제가 `on delete restrict`로 막힌다.** 지우고 다시 만들 수도 없는 상태가 된다

> **선행은 모두 끝났다** — T1.12-7(반영 지연 최대 1시간 → 약 1초)과 `main` 머지. T1.12-7 전에는 카드를 고쳐도 상세가 옛 내용을 보여줘 T1.14의 입력 검증 자체가 성립하지 않았다.

- [ ] **T1.13** 카탈로그 로컬 덤프 — `npm run db:dump` · S
  ...
  - **완료 기준:** `npm run db:dump`가 파일을 만들고, **그 파일로 로컬(`db:reset`) DB를 복원해 `card_sets`·`cards`·`keywords`·`card_keywords`·`news_posts` 행 수가 원격과 일치하는 것을 1회 실측한다.** 덤프 파일이 `git status`에 뜨지 않는다
    - ⚠️ **리허설 전에 표본을 먼저 넣는다(`npm run db:sample`).** 카탈로그가 0행이라 그냥 돌리면 **"0 = 0"을 확인하는 무의미한 리허설**이 된다 — 덤프가 깨져 있어도 통과한다. 이번엔 세트 2 · 카드 8 · 키워드 3 · 기사 3으로 피했고, **다음에 리허설을 다시 할 때도 같은 함정이 있다**

- [ ] **T1.14** 실제 카드 손입력 1배치 — 원피스 ST-01 · ST-02 (34종) · M
  - **선행: T1.13(되돌릴 수단) · T1.15(오타를 고칠 화면). 실행은 사용자가 직접 한다.** 목표는 카탈로그를 채우는 것이 아니라 **측정하는 것**이다 — 아래 "측정 기록"을 채우는 것이 산출물이고, 그 값이 A-5 · B-4 · B-5의 순위를 정한다
...
- [ ] **T1.15** 세트 · 키워드 수정 · 삭제 (백로그 A-4) — **M** · **마이그레이션 0건** · `T1.14`보다 **먼저**
  - **왜 앞으로 당겼나:** 손입력 도중 오타를 발견하는 시점이 문제다. 카드를 넣기 *전*이라면 세트·키워드를 지우고 다시 만들면 되지만, **넣는 도중이면 `cards.set_id`의 `on delete restrict`가 세트 삭제를 막는다.** 수정 화면이 없으면 그 세트는 영영 그 이름으로 남는다. plan.md가 이미 "손입력에서 막히면 A-4"를 첫 분기로 적어 두었다
  - **스키마 확인 완료 — 마이그레이션이 필요 없다.** `service_role`은 001의 GRANT로 `card_sets`·`keywords`에 UPDATE·DELETE를 이미 갖고 있다. 새 테이블·컬럼·제약이 없다

  **ⓐ 재사용 — 카드(T1.12-2)의 형태를 그대로 따라간다. 달라지는 지점만 아래 ⓑ~ⓓ다**
  - `AdminDeleteButton`(`endpoint` · `redirectTo` · `label`)과 `useAdminForm`의 등록·수정 겸용 분기(`endpoint` 삼항 · `method: "PATCH"` · `resetOnSuccess: !isEdit`)는 이미 일반화돼 있다. `SetForm`·`KeywordForm`에 `setId?`/`keywordId?` + `initial?`을 더하는 것이 전부다
  - 새 라우트 `PATCH·DELETE /api/admin/sets/[setId]` · `/api/admin/keywords/[keywordId]`는 `requireAdminInput()` + 기존 zod 스키마(`setInputSchema` · `keywordInputSchema`)를 그대로 쓴다
  - ⚠️ **수정 모드에서 `game_id`는 잠근다(읽기 전용 표시).** 세트의 게임을 바꾸면 `cards`의 복합 FK `(set_id, game_id)`가 깨져 DB가 막지만 **에러 메시지가 원인을 설명하지 못하고**, 키워드는 더 나쁘다 — FK가 게임을 참조하지 않아 **에러 없이** 다른 게임 카드에 태그가 붙은 채로 남아 필터가 조용히 어긋난다. 애초에 바꿀 수 없게 하는 편이 싸다

  **ⓑ 삭제의 파괴 범위가 카드와 다르다 — 두 방향으로 다르다**

  | 대상 | DB 동작 | 위험 | 화면 처리 |
  |------|---------|------|-----------|
  | **세트** | `cards.set_id`가 `on delete restrict` → **삭제가 실패한다** | 낮다(막아 준다). 문제는 **왜 실패했는지 안 보이는 것** | 삭제 전에 **참조 카드 수를 세서** 0이 아니면 `409`로 막고 **"카드 N장이 이 세트를 쓰고 있습니다"**를 준다 |
  | **키워드** | `card_keywords`가 `on delete cascade` → **조용히 성공한다** | **높다.** 카드 N장의 태그가 말없이 사라지고 도감 필터에서 그 카드들이 빠진다 | 확인 단계에서 **"카드 N장에 붙어 있습니다. 지우면 태그가 함께 사라집니다"**를 먼저 보여 준다 |

  - ⚠️ **`databaseError`의 `23503` 매핑을 그대로 쓰면 안 된다.** 현재 문구가 `"선택한 세트가 이 게임에 속하지 않습니다."`인데 이건 **insert 상황의 말**이고, 세트 삭제 실패에 나오면 뜻이 통하지 않는다. 같은 PG 코드가 두 상황을 가리키므로 **`databaseError`에 호출부별 override를 받는 선택 인자를 더한다**(`responses.ts`, 가산 변경 · 단위 테스트 1건 추가). 사전 카운트는 UX를 위한 것이고 **진짜 방벽은 여전히 DB 제약**이다 — 경합으로 뚫려도 23503이 override된 문구로 나온다
  - `AdminDeleteButton`에 **`description?: string`** prop을 더해 확인 단계에 경고 한 줄을 띄운다. **새 컴포넌트를 만들지 않는다** — 카드·뉴스는 이 prop을 넘기지 않으면 지금과 똑같이 동작한다
  - 참조 카드 수는 페이지(RSC)가 `count: "exact", head: true`로 세어 내려보낸다. `fetchCounts`가 쓰는 패턴 그대로다

  **ⓒ 목록 → 수정 경로 — 별도 목록 라우트를 만들지 않는다**
  - `/admin/sets` · `/admin/keywords`는 **이미 폼 아래에 전체 목록을 렌더한다.** 각 `<li>`를 `/admin/sets/[setId]` · `/admin/keywords/[keywordId]`로 링크하면 끝이다
  - 카드가 별도 목록(`/admin/cards`)을 가진 이유는 **수백 장이 되기 때문**이고, 그것이 T1.12에서 A-4를 뺀 근거이기도 했다. **세트·키워드는 손입력 뒤에도 수십 개 규모**라 그 전제가 그대로 유효하다. `Pagination`도 검색창도 넣지 않는다 — 필요해지면 그때 `/admin/cards`를 복사한다

  **ⓓ 캐시 — §2.7의 "쓰기 전에 방문" 규칙은 여기에 적용되지 않는다. 대신 다른 함정이 있다**
  - `/admin/**`은 전부 `ƒ`이고, 세트·키워드가 비치는 공개 화면도 캐시가 없다 — `/cards/[cardId]`는 `ƒ`(T1.12-7), `/cards` 목록과 필터는 Route Handler(`/api/cards`·`/api/cards/facets`) 조회다. **ISR로 세트·키워드를 읽는 곳이 없다**
  - **대신 TanStack Query가 있다** — 검색 결과 `staleTime` 5분, 패싯 60초. 같은 탭에서 키워드 이름을 바꾸고 곧바로 도감 칩을 확인하면 옛 이름이 남는다. **E2E는 새 `page.goto()`로 앱을 다시 띄워 확인한다**(페이지 로드가 QueryClient를 새로 만든다)
  - **`revalidateCards()`는 세트 쓰기에만 붙인다.** 홈(`○ 10m`)의 `fetchCatalogStats`가 **`card_sets`를 센다** — 세트를 지우면 홈의 수치가 10분 낡는다. 키워드는 ISR 페이지 어디서도 읽지 않으므로 붙이지 않는다. **이 비대칭은 의도된 것이다**(붙이는 쪽과 안 붙이는 쪽 모두 근거가 있다 — 습관으로 복사하지 말 것)

  **ⓔ 크기와 순서 — 커밋 2개로 나눈다**
  - **T1.15a 세트 · M** — 공용 확장(`databaseError` override · `AdminDeleteButton`의 `description`)을 여기에 포함한다
  - **T1.15b 키워드 · S** — a의 템플릿을 그대로 따라가므로 훨씬 빠르다
  - **시간이 하나만 허용하면 세트(a)를 한다.** 카드가 걸린 뒤 되돌릴 수 없는 쪽이 세트다(위 ⓑ 표). 키워드는 지웠다 다시 붙일 수 있다

  - **완료 기준**
    1. **세트**: 등록 → 목록에서 클릭 → `name_ja` 수정 → 저장 → 목록에 반영. **카드가 걸린 세트를 지우려 하면 409 + 카드 수가 담긴 문구**가 뜨고 세트는 그대로 남는다. 카드가 없는 세트는 지워지고 목록에서 사라진다
    2. **키워드**: 등록 → 수정 → 삭제. **확인 단계에 "카드 N장" 경고**가 뜬다. 삭제 후 그 카드들의 상세에서 칩이 사라진다(새 `page.goto()`로 확인)
    3. `test`에 `databaseError` override 1건 추가. E2E 2건(세트 왕복 · 키워드 왕복)이며 **각 spec은 마지막에 자기 데이터를 지운다**(§9.9 선례)
    4. ⚠️ **새 spec이 새 접두사를 쓰면 `scripts/cleanup-sample.ts`의 패턴과 표를 같은 커밋에서 갱신한다** — §9.9 규칙 1. T1.12-7이 이걸 어겨 `unpub-`가 남았다
...
- [ ] **A-4** 세트 · 키워드 수정 · 삭제 → **T1.15로 승격했다.** T1.12에서 뺀 근거("개수가 적고 다시 못 찾는 문제가 아니다")는 여전히 맞지만, **손입력 도중이면 `on delete restrict`가 세트 삭제를 막아 되돌릴 방법이 없어진다** — 그래서 T1.14보다 앞으로 당겼다
...
**순서: ① `main` push → ② `ADMIN_TOKEN` 교체 → ③ T1.13 덤프 → ④ T1.15 세트·키워드 수정 → ⑤ T1.14 손입력.** ①②는 각각 5분이고, **③④가 ⑤의 착수 조건**이다.

- **① `origin`에 push.** 로컬 `main`이 5커밋 앞서 있다. 백업이 로컬에만 있는 상태를 하루 넘기지 않는다
- **② `ADMIN_TOKEN` 43자 난수로 교체** (§9.2 ⓐ의 명령 그대로). **데이터가 들어가기 전에** 끝내야 하는 항목이다
- **③ T1.13 — 되돌릴 수단.** §9.2 ⓑ. 무료 플랜에 자동 백업이 없어 이것이 유일한 복구 경로다. **복원 리허설 1회까지가 완료 기준**이고, 리허설 전에 `db:sample`로 표본을 넣어야 의미가 있다
- **④ T1.15 — 세트·키워드 수정·삭제.** 손입력 도중 오타를 발견하면 **그때는 카드가 걸려 세트를 지울 수도 없다.** 하나만 한다면 세트(a) 쪽이다
- **⑤ T1.14 — 손입력 1배치(ST-01 · ST-02 34종). 사용자가 직접 한다.** 목표는 채우는 것이 아니라 **측정**이다
...
|---|---|
| ~~세트명 · 키워드 오타를 고칠 화면이 없어 막힌다~~ | **선제 해소 — T1.15를 앞으로 당겼다.** 이 분기가 실현되면 이미 늦기 때문이다 |
