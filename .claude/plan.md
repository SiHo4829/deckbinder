...
> **이 절이 확정하지 않는 것 — 원천의 HTML 태그 구조다.** 실측한 것과 확인하지 못한 것을 **ⓙ에 갈라 적었다.** ⚠️ **ⓙ의 「미확인」을 「없다」나 「허용」으로 읽지 않는다** — §4.4.1이 미끄러진 자리가 정확히 그 형태(못 찾음을 허용으로 읽음)다.
...
|---|--------|------|------|------------------------------------------|
| **T1.16** | **수집기** — 원천 → 중간 파일 | **M~L** | 없음 (T1.13 · `CLAUDE.md` 개정 완료) | 중간 파일이 만들어지고, **ⓙ의 미확인 항목이 실측으로 채워진다** |
| **T1.17** | **마이그레이션 008** — 카드 성능 컬럼 | **M** | T1.16 + **중간 파일을 사람이 눈으로 확인** | 스키마 공백 **셋**이 닫힌다 (ⓗ) |
...
| 첫 실행 권장값 | **1세트 · 8페이지 · 상한 12** | ⓙ에서 OPK-14의 마지막 페이지 인덱스가 7(= 8페이지)임을 확인했다. 여유 4는 재시도분 |
...
| 상황 | 처리 | 근거 |
|------|------|------|
| **중복** — 같은 `code`가 DB에 이미 있다 | **skip + 리포트.** 실패로 세지 않는다 | 반복 실행이 전제다. 이것을 에러로 만들면 두 번째 실행이 항상 빨갛게 끝나 **사람이 리포트를 안 읽게 된다** |
| 🚨 **중간 파일 *안*에 `code`가 중복** | **전건 중단** | 파싱이 잘못됐다는 신호다 — 같은 카드를 두 페이지에서 긁었거나 코드 추출이 틀렸거나, **패러렐 카드가 같은 코드로 나온 것**이다. 마지막 경우 §4.6의 `_p1` 접미사를 **누가 붙이는지**가 미결이고(ⓙ), **임포터가 자동으로 붙이지 않는다.** 사람이 규칙을 정할 자리다 |
...
| **중간 파일 하나에 세트가 둘 이상** | **거부한다.** 중간 파일 1개 = 세트 1개를 강제한다 | 수집기가 세트 단위로 도는데(ⓔ) 파일이 섞이면 매핑을 추론해야 한다. **추론하지 않기 위한 제약이다** |
| `code` 접두사가 `--set`과 어긋난다 (프로모 혼입 등) | **경고로 표시하고 건수를 센다. 차단하지는 않는다** | 코드 규칙은 우리가 정한 것이 아니다. 사람이 리포트를 보고 판단한다 |
| **필수 컬럼 결측** — `code` | **전건 차단** | |
| 🚨 **필수 컬럼 결측 — `name_ja`** | **전건 차단. 그리고 한국어명으로 채우는 경로를 코드에 만들지 않는다** | **이 설계에서 가장 위험한 지점이다 — 아래 별도 항목** |
...
- `cards.name_ja`는 **`not null`**이고 §5.3 매물 크롤러의 **유일한 검색 키**다(§4.4의 제약 표 · §0.1 ⓓ가 T1.14에서 「살아남는 것 셋」 중 첫째로 옮겨 적었다).
- 유일 원천은 **한국 사이트**이고, ⓙ의 관측 범위에서 **일본어명을 주는 것을 보지 못했다.** ⚠️ **못 봤다는 것이 없다는 뜻은 아니다 — T1.16의 첫 실측 항목이다.**
...
- **→ 장치 셋.**
  ...
  2. 중간 파일에 `name_ja`가 없으면 그 행은 **`invalid`**이고, 하나라도 있으면 **`--apply`를 거부한다.**
  3. 그러면 1차 적재가 통째로 막힌다 — **그것을 T1.17이 판정한다.** 선택지는 셋이고 **지금 고르지 않는다**(재료가 없다): ⓐ `name_ja`를 nullable로 되돌리고 T2.7이 null을 다룬다 ⓑ 채울 원천을 따로 둔다(§9.3 ⓑ 기준으로 **카드 실물**이 유일하게 열린 경로다 — 일본 사이트는 §4.4.1 결정 2가 닫았다) ⓒ 「미확보」를 표시할 자리를 만든다.
...
#### ⓗ 스키마 공백 4건 — **순서를 확정하고, 넷을 셋 + 하나로 가른다**
...
| 2 | **opcg 다색을 단일 `attribute` 컬럼으로 못 담는다** | §4.7 ⓗ-2 | ✅ | ⓙ에서 목록에 **색**이 나오는 것을 확인했다(`적색`). 2색 카드가 어떻게 표기되는지는 중간 파일이 답한다 |
| 3 | **코스트 · 파워 · 카운터를 담을 컬럼이 없다** | §9.4 ⓐ | ✅ | ⓙ에서 목록에 **코스트와 파워가 실제로 나오는 것**을 확인했다 |
| 4 | **원피스 特徴을 `sub_type`에 넣을 수 없다** | T1.14 (§0.1 ⓓ) | ✅ | ⓙ에서 特徴(`초신성` · `하트 해적단`)이 나오는 것을 확인했다 |
...
**T1.17이 함께 판정하는 것 하나 더 — ⓕ의 `name_ja`다.** 스키마 공백 4건과 같은 마이그레이션에서 결정된다(선택지 셋은 ⓕ 참조).
...
#### ⓙ 원천 실측 — **확인한 것과 확인하지 못한 것 (2026-08-28)**

**확인 방법:** 에이전트 도구로 아래 URL을 **총 3회** 열었다(`robots.txt` 1 · `cardlist.do` 2). ⓔ의 부하 규율에 어긋나지 않는 범위다.
...
| 세트 라벨 형식 | `[OPK-14] 부스터 팩 창해의 칠걸`. 선택지는 **28개 이상**(부스터 팩 + 스타터 덱, `[OPK-01]`까지) | 같은 URL · 2026-08-28 |
| 🚨 **세트 라벨과 카드 코드가 어긋난다** | 라벨은 **`OPK-14`**인데 카드 코드는 **`OP14-001`**이다 | 같은 URL · 2026-08-28 |
...
| # | 미확인 항목 | 왜 중요한가 |
|---|-------------|-------------|
| 1 | 🚨 **일본어 카드명을 어디서든 주는가** | **가장 중요하다.** `cards.name_ja`가 `not null`이고 §5.3의 유일한 검색 키다 → ⓕ의 🚨 |
| 2 | 🚨 **패러렐 · 얼터 아트가 목록에 어떤 코드로 나오는가** | 같은 코드로 여러 행이 나오면 §4.6의 `_p1` 접미사를 **누가 붙이는가**가 열린다. 임포터는 **자동으로 붙이지 않는다**(ⓕ) |
| 3 | **카드 상세 페이지가 있는가 · 그 URL 패턴** | 링크 조회에서 나오지 않았으나 **없다는 확증이 아니다.** 목록이 필드를 다 담고 있어 **없어도 성립한다** |
| 4 | **이미지 URL 패턴** | §9.4의 핫링크 대상이 이 값이다. B-6 폴백과 §9.4 ⓔ 측정이 여기 걸린다 |
| 5 | **2색 카드의 색 표기 방식** | ⓗ-2가 이 값으로 판정된다 |
| 6 | **실제 HTML 태그 구조** | 취득 도구가 마크다운으로 변환해 **원본 마크업을 보지 못했다.** 파서와 합성 픽스처(ⓘ)가 여기서 시작한다 |
| 7 | **서버 렌더링인지** | 정적 취득으로 본문이 나왔으므로 **정황은 강하나**, 도구가 JS를 실행하지 않는다는 것을 문서로 확인하지 않았다. §3.3(`docs/crawler-compliance.md`)의 2026-08-23 관측(「서버 렌더링 HTML」)과는 일치한다 |
| 8 | **`size`의 상한** | ⓔ대로 **기본은 관측값 20을 그대로 쓴다.** 키우는 판단은 근거를 적고 승인을 받는다 |
| 9 | **응답 헤더 · rate limit 신호** | 상대가 부하에 대해 무엇을 말하는지 알 수 있는 유일한 남은 채널이다 |
| 10 | **세트 셀렉터의 전체 목록과 세트별 페이지 수** | ⓔ의 요청 상한을 세트별로 정하는 근거 |
...
- [~] **T1.14** 실제 카드 손입력 1배치 — **폐기 (2026-08-28 · §0.1 ⓓ).** 사용자가 자동 수집으로 방침을 바꿨다
  ...
  - **살아남는 것 셋 — 수집기가 그대로 받는다.**
    - **`name_ja`가 `not null`이다**(§4.1). 원피스 한국 사이트는 일본어명을 주지 않을 수 있고, 그때 이 컬럼을 무엇으로 채울지가 열려 있다. **한국어명을 넣어 두면 §5.3의 매물 검색 키가 오염된다** — 그 크롤러가 `name_ja`로 일본 사이트를 검색하기 때문이다
...
- [ ] **T1.16** 카탈로그 수집기 — 원천 → 중간 파일 · **M~L** · 선행 없음(T1.13 · `CLAUDE.md` 개정 완료)
  - 🚨 **첫 단계는 코드가 아니라 실측이다.** §4.8 ⓙ의 미확인 10건, 특히 **①일본어명 유무**와 **②패러렐 코드 표기**를 먼저 연다. **그 둘이 T1.17과 T1.18의 입력이다** — 모른 채 파서를 쓰면 §4.4.1이 미끄러진 것과 같은 방식으로(못 찾음을 없음으로 읽음) 잘못된 값이 들어간다
  - **완료 기준**
    ...
    - ⓗ 재실행이 이미 받은 **(세트, 페이지)**를 다시 받지 않는다 (§0.1 ⓓ ⓒ)
    - ⓘ **실제 응답 1페이지에 파서를 돌린 결과를 사람이 원본 화면과 대조한다.** 산출물은 커밋하지 않는다
    - ⓙ **§4.8 ⓙ의 미확인 표가 실측 결과로 갱신된다** — 확인일과 함께. 확인하지 못한 것은 그대로 미확인으로 남긴다
...
- [ ] **T1.17** 마이그레이션 008 — 카드 성능 컬럼 · **M** · 선행 **T1.16 + 중간 파일을 사람이 눈으로 확인**
  - **완료 기준**
    - ⓐ 스키마 공백 **셋**(다색 · 코스트/파워 · 特徴)을 닫는다. **ptcg 기본 포켓몬(§4.7 ⓗ-1)은 닫지 않는다** — 근거는 §4.8 ⓗ
    - ⓑ **판정 근거를 §4.8 ⓗ 표에 중간 파일의 실제 값과 함께 적는다.** 추측으로 컬럼을 만들지 않는다
    - ⓒ **`sub_type`에 特徴을 넣지 않는다** (§4.0이 `basic_energy` 전용으로 못박았고 特徴은 여럿이다)
    - ⓓ **`name_ja` 처리를 함께 판정한다** — §4.8 ⓕ의 선택지 셋 중 하나. **「한국어명을 넣는다」는 선택지가 아니다**
...
- [ ] **T1.18** 카탈로그 임포터 — 중간 파일 → DB · **L** · 선행 T1.16 · T1.17
  - **완료 기준**
    ...
    - ⓘ 중간 파일 안에 `code` 중복이 있으면 **전건 중단**한다
    - ⓙ 🚨 **`name_ja` 결측이 하나라도 있으면 전건 차단한다. 한국어명으로 채우는 경로가 코드에 없다**
    - ⓚ 분류기가 단위 테스트된다 — `insert`/`skip`/`update`/`invalid` 4종 + 세트 부재 + 코드 중복 + `name_ja` 결측 + 연속 10건 실패 중단
...
- [ ] **T1.19** 첫 실적재 1세트 + 측정 — **S (단 사람 손이 필요하다)** · 선행 T1.18 + 사용자 일감(범위 승인 · `db:dump` · §9.3 ⓓ 재확인)
  ...
  - **완료 기준**
    ...
    - ⓕ 실행 기록을 이 블록에 한 줄로 남긴다 — **날짜 · 대상 세트 · 총 요청 수 · 실패 수 · 적재 행 수** (§4.8 ⓓ — 매니페스트 파일은 커밋하지 않으므로 **이것이 유일한 추적 가능한 기록이다**)
...
- [ ] **A-5** ~~CSV 일괄 등록~~ → **카탈로그 자동 수집 + 임포트 · L~XL** — ★ **2026-08-28에 성격이 바뀌어 최우선이 됐고**(§0.1 ⓓ), **같은 날 설계가 확정돼 T1.16~T1.19로 쪼개졌다**(§4.8). T1.14가 폐기되면서 **이것이 카탈로그를 채우는 유일한 경로**다
  ...
  - ⚠️ **닫히지 않은 것 하나 — 원천의 실제 HTML 구조다.** §4.8 ⓙ에 **확인한 것 8건과 미확인 10건**을 갈라 적었다. **미확인을 「없음」으로 읽지 않는다** — T1.16의 첫 단계가 그것을 여는 일이다
...
| ★ **다음 세션 1순위 (에이전트) — 2026-08-28 갱신** | **T1.16 (카탈로그 수집기).** 설계는 **§4.8**에 확정됐고 로드맵 블록에 완료 기준이 있다. 🚨 **첫 단계는 코드가 아니라 §4.8 ⓙ의 미확인 10건 실측**이다 — 특히 **일본어명 유무**와 **패러렐 코드 표기**. 그다음이 **E-2**(저비용 · 독립) |
...
| 4 | ~~T1.14 손입력~~ · ~~`CLAUDE.md` 개정 승인~~ → **둘 다 닫혔다 (2026-08-28).** 남은 사용자 몫은 **A-5 수집 범위 승인**(어느 세트부터, 몇 장까지) | 개정은 반영됐다(`CLAUDE.md` §2 (B)). **수집을 실제로 돌리기 전에 범위를 사람이 정해야 한다** — 부하 규율(§4.8 ⓔ)의 「전체 요청 수 상한」이 그 값에서 나오고, **`--max-requests`가 없으면 수집기가 시작을 거부한다.** ★ **권장 첫 실행: 1세트(`[OPK-14]`) · 8페이지 · 상한 12** (§4.8 ⓙ에서 페이지 수를 확인했다) |
| 4b | **수집기 User-Agent에 연락처를 넣을지, 넣는다면 어떤 주소를 쓸지** | **개인 주소를 외부 서버에 헤더로 보내는 판단이라 사용자 몫이다**(§4.8 ⓔ). ⚠️ **넣는 것이 「묻는 것」은 아니다 — 방향이 반대다.** §0.1 ⓒ가 폐기한 것은 허락을 구하는 행위이고, UA 연락처는 **상대가 멈추라고 말할 수 있게 하는 것**이다. §9.11 ⓖ대로 외부 접촉이 「중지 요청」 형태로만 올 수 있는 지금, **연락처가 없으면 그 유일한 경로가 좁아진다.** 기본값은 연락처 없는 `DeckBinder-CatalogBot/0.1` |
